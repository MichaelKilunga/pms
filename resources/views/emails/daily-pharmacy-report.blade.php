...
<body>
    <div class="container">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>TZS {{ number_format($salesSummary['gross_profit'], 2) }}</td>
                    </tr>
                    <tr>
                        <td>Total Expenses</td>
                        <td>TZS {{ number_format($salesSummary['total_expenses'], 2) }}</td>
                    </tr>
                    <tr>
                        <td>Total Installments (Debt Payments)</td>
                        <td>TZS {{ number_format($salesSummary['total_installments'], 2) }}</td>
                    </tr>
                    <tr>
                        <td>Total Net Profit/Loss</td>
            ...
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>{{ $stockStatus['out_of_stock']->count() > 0 ? round(($stockStatus['out_of_stock']->count() / ($stockStatus['out_of_stock']->count() + $stockStatus['low_stock']->count() + $stockStatus['expired']->count() + $stockStatus['good_stock']->count())) * 100, 1) : 0 }}%
                        </td>
                    </tr>
                    <tr>
                        <td class="status-warning">Low Stock</td>
                        <td>{{ $stockStatus['low_stock']->count() }}</td>
                        <td>{{ $stockStatus['low_stock']->count() > 0 ? round(($stockStatus['low_stock']->count() / ($stockStatus['out_of_stock']->count() + $stockStatus['low_stock']->count() + $stockStatus['expired']->count() + $stockStatus['good_stock']->count())) * 100, 1) : 0 }}%
                        </td>
                    </tr>
                    <tr>
                        <td class="status-danger">Expired</td>
                        <td>{{ $stockStatus['expired']->count() }}</td>
                        <td>{{ $stockStatus['expired']->count() > 0 ? round(($stockStatus['expired']->count() / ($stockStatus['out_of_stock']->count() + $stockStatus['low_stock']->count() + $stockStatus['expired']->count() + $stockStatus['good_stock']->count())) * 100, 1) : 0 }}%
                        </td>
                    </tr>
                    <tr>
                        <td class="status-good">Good Stock</td>
                        <td>{{ $stockStatus['good_stock']->count() }}</td>
                        <td>{{ $stockStatus['good_stock']->count() > 0 ? round(($stockStatus['good_stock']->count() / ($stockStatus['out_of_stock']->count() + $stockStatus['low_stock']->count() + $stockStatus['expired']->count() + $stockStatus['good_stock']->count())) * 100, 1) : 0 }}%
                        </td>
                    </tr>
                </tbody>
            </table>
                        ...
                        @foreach ($stockStatus['expired'] as $stock)
                            <tr>
                                ...
                                <td class="status-danger">
                                    {{ \Carbon\Carbon::parse($stock->expire_date)->format('M j, Y') }}</td>
                                <td>{{ $stock->batch_number }}</td>
                                <td>{{ $stock->supplier }}</td>
                            </tr>
                        @endforeach
        ...
        <div class="footer">
            ...
            <p>© {{ $pharmacy->name }} - {{ config('app.name') }} <br> <span style="color: #dc3545">Pharmacy
