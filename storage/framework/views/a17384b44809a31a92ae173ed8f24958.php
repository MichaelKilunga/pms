<?php $__env->startSection("content"); ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.sale-entry').forEach(row => {
                // Initialize date from filter if available
                if ($('#saleDateFilter').length) {
                    $(row).find('.date').val($('#saleDateFilter').val());
                }
        ...
        $(document).ready(function() {
            ...
            $('#saleDateFilter').on('change', function() {
                var selectedDate = $(this).val();
                table.ajax.reload();
                // Update existing row dates in the modal if it's open
                $('.sale-entry .date').val(selectedDate);
            });
        });
    </script>
